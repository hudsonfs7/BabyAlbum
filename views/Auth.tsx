...
import { 
  Baby, Heart, Mail, Lock, Camera, ArrowRight, 
  ArrowLeft, Check, Stars, Sparkles, Cloud, Loader2, Smile, LogIn 
} from 'lucide-react';
...
export const Auth: React.FC<{ onAuthSuccess: () => void }> = ({ onAuthSuccess }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col p-6 max-w-md mx-auto relative overflow-hidden bg-white">
      {/* Background Decor */}
      ...
      {/* LOGIN STEP */}
      {step === 1 && (
        <div className={`${VISUAL_STANDARDS.card} flex-1 flex flex-col p-8 bg-white/95 shadow-2xl animate-in slide-in-from-right duration-500`}>
      ...
      {step >= 2 && (
        <div className={`${VISUAL_STANDARDS.card} flex-1 flex flex-col p-8 bg-white/95 shadow-2xl`}>
          {/* STEP 2: ROLE (Register Start) */}
          {step === 2 && (
          ...
          {step === 3 && (
            <div className="flex-1 flex flex-col animate-in slide-in-from-right duration-500">
              ...
              <div className="grid grid-cols-2 gap-4 flex-1 overflow-y-auto pb-4 px-1">
                {PERSONAS.map(p => (
                  <button 
                    key={p.type}
          ...
          {/* STEP 6: PHOTOS */}
          {step === 6 && (
            <div className="flex-1 flex flex-col animate-in slide-in-from-right duration-500 text-center">
               <button onClick={prevStep} className="absolute top-6 left-6 opacity-30"><ArrowLeft size={20}/></button>
